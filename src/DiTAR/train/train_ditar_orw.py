...
from DiTAR.model import DiTAR_ORW_Trainer, DiTAR_ORW
...
def create_model(model_cfg, tokenizer):
    return DiTAR_ORW(ditar_cfg=model_cfg, tokenizer=tokenizer)
...
def main(cfg):
    ...
    val_dataset = None  # ORW doesn't run validation forward (rollout is too costly).
