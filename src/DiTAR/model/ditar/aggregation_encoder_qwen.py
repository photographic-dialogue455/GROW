from DiTAR.model.backbones.qwen3 import Qwen3Model, Qwen3Config
import torch.nn as nn
import torch
from typing import Optional

class AggregationEncoder_Qwen(nn.Module):
    def __init__(
        self,
        hidden_size: int,
    ...
    def forward(
        ...
        attention_mask = self.to_4d(attention_mask, attention_mask.shape[1],key_value_length=attention_mask.shape[1],dtype=x.dtype)
        
        outputs = self.model(inputs_embeds=inputs_embeds, attention_mask = attention_mask)
        hidden_states = outputs.last_hidden_state
        
        # Extract summary tokens
        summary_tokens = self.get_summary_token(hidden_states).view(B, num_patches, D) #(B,num_paches,1024)

        # Compute final padding mask
        padding_mask = padding_mask.view(B, num_patches, -1) #(B,num_patches,patch_size)
        summary_padding_mask = torch.sum(padding_mask, dim=-1) > 0 #(B,num_patches) 
