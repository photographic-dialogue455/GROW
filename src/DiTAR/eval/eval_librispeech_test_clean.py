...
def get_args():
    ...
    parser.add_argument("--metalst", type=str, default="", help="Optional manifest (.lst) override; empty = full hardcoded list")
...
def main():
    ...
    metalst = args.metalst if getattr(args, "metalst", "") else (rel_path + "/data/librispeech_pc_test_clean_cross_sentence.lst")
    print(f"[eval] using metalst = {metalst}")

    gpus = parse_gpu_nums(args.gpu_nums)
    if "LibriSpeech" in librispeech_test_clean_path:
    ...
    if local:  # use local custom checkpoint dir
        asr_ckpt_dir = "checkpoints/faster-whisper-large-v3"
    else:
        asr_ckpt_dir = ""  # auto download to cache dir
    wavlm_ckpt_dir = "checkpoints/wavlm_large_finetune.pth"
